...
public class GridBagLayoutEX03 extends JFrame {
	...
	GridBagLayoutEX03() {
		...
		gbc.weightx=0.8; //設定weightx的比重為0.8
		cp.add(new JButton("Button One"), gbc); //將按鈕加到版面

		gbc.gridx = 1; gbc.gridy = 0; //設定元件的行列位置
		gbc.weightx=0.2; //設定weightx的比重為0.2
		cp.add(new JButton("2"), gbc);

		gbc.weightx=0.0; //恢復預設值

		/***加入第二列的元件***/
		gbc.ipadx = gbc.ipady = 0;
		//設定元件內文字與四周邊界的間距均為0

		gbc.anchor = GridBagConstraints.WEST; //設定第二列均為靠西(左)對齊
		...
		/***加入第三列的元件***/
		gbc.anchor = GridBagConstraints.EAST; //設定第三列均靠東(右)對齊
		...
		gbc.anchor = GridBagConstraints.SOUTH; //靠南(下)對齊
		cp.add(new JButton("Button 7"), gbc);

		gbc.gridx = 1; gbc.gridy = 3; //設定元件的行列位置
		gbc.weighty = 1.0; //設定weighty的比重為1.0
		gbc.anchor = GridBagConstraints.NORTH; //靠北(上)對齊
	...
	public static void main(String args[]) {
		new GridBagLayoutEX03(); //宣告建立視窗畫面的frame物件
	}
}