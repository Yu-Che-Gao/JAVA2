	...
	public Component getTableCellRendererComponent(
		...
		if(hasFocus){
			...
			for(int i=0; i < table.getColumnCount(); i++){
				...
				if(i != column){ //不重繪是否發行欄位的儲存格, 避免遞迴重繪(此行敘述雖然正確但無法避免不被重繪)
					Rectangle r = table.getCellRect(row, i, false); //取得同一列其他欄位的儲存格範圍
					table.repaint(r); //重繪儲存格
					System.out.println("重繪 (" + row + ", " + i + ") 儲存格");
				}
			}
			/*
			if(!repaint){
